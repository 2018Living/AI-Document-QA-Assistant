...
### 项目一：RAG 文档问答助手
- **背景**：大模型无法回答私有文档内容，需要构建 RAG 系统
- **实现**：Streamlit + PyPDF + 智谱 AI，通过关键词检索实现RAG
- **成果**：支持 PDF/TXT 上传，能给予文档内容回答问题
- **亮点**：实现了中英文混合分词，修复了关键词提取 bug

### 项目二：Agent智能体
- **背景**：大模型无法调用外部工具，需要 Agent 架构
- **实现**：LangGraph + Tavily Search + 智谱 AI
- **成果**：Agent 能自动调用计算器、获取时间、搜索真实新闻
- **两点**：解决了无参数工具调用的兼容问题
